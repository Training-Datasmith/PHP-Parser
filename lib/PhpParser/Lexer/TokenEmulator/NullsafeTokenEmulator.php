...
final class Nullsafe_Token_Emulator extends Token_Emulator
{
    ...
    public function emulate(string $code, array $tokens): array
    {
        ...
        for ($i = 0, $c = count($tokens); $i < $c; ++$i) {
            ...
            if ($token->id === \T_ENCAPSED_AND_WHITESPACE && isset($tokens[$i - 1]) && $tokens[$i - 1]->id === \T_VARIABLE && preg_match('/^\?->([a-zA-Z_\x80-\xff][a-zA-Z0-9_\x80-\xff]*)/', $token->text, $matches)) {
                ...
                if ($match_len !== \strlen($token->text)) {
                    $replacement[] = new Token(\T_ENCAPSED_AND_WHITESPACE, \substr($token->text, $match_len), $token->line, $token->pos + $match_len);
                }
