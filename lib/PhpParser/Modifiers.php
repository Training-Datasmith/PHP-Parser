...
final class Modifiers
{
    ...
    public static function to_string(int $modifier): string
    {
    ...
    private static function is_valid_modifier(int $modifier): bool
    {
        $is_pow2 = ($modifier & $modifier - 1) == 0 && $modifier != 0;
        return $is_pow2 && $modifier <= self::PRIVATE_SET;
    }
    /**
     * @internal
     */
    public static function verify_class_modifier(int $a, int $b): void
    {
        assert(self::is_valid_modifier($b));
        if (($a & $b) != 0) {
            throw new Error('Multiple ' . self::to_string($b) . ' modifiers are not allowed');
        }
        if ($a & 48 && $b & 48) {
     ...
     * @internal
     */
    public static function verify_modifier(int $a, int $b): void
    {
        assert(self::is_valid_modifier($b));
